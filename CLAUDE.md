...
3. **Go AI server** (`ia_server/`) — censorship/moderation, scenario→session bootstrap, word2vec lookups, OMWfr/Wiktionnaire dictionaries. Endpoints: `/api/health`, `/api/checkMsg`, `/api/newSession`, `/api/classify` (scripted intent classifier — Phase 6 word2vec, Phase 7 ajoute un backend LLM Mistral derrière la même route ; switch interne via `CLASSIFY_BACKEND=word2vec|llm`. Voir `ia_server/pkg/classify/{classify,llm}.go` + `ia_server/webservice/classify.go`). The Svelte side degrades gracefully if `IA_SERVER_URL` is unset or unreachable.
...
- `scenario/` — mixed: `index.ts` holds the legacy free-engine CRUD (`createScenario`, `createEventsAndEnds`) usable both client- and server-side. The six scripted helpers are **server-only** (most use `MyPocketBase` instances built from the private `DB_URL`, or take `pb` as an argument so they remain testable under Vitest) :
...
- `server/ia/` — the only IA client code; kept server-side so the private `IA_SERVER_URL` never reaches the bundle. Includes `classify.ts` (adapter SvelteKit — Phase 6/7) + `classify-core.ts` (cœur hermétique testable, propage `classification` 3036 quand le payload Go le pose, Phase 7).
...
- `End` gained `condition`, `priority`, `external_id`.
...
**Runtime + classifier injection point** (Phase 5 / Phase 6 / Phase 7) — when a player submits a turn on a scripted session, `+page.server.ts:addNode` routes to `progressScripted(pb, sessionId, text, classifier)` (cf. `$lib/scenario/runtime-scripted.ts`). Le classifier est sélectionné par `pickClassifier()` (cf. `$lib/server/ia/classify.ts`) selon la variable d'environnement privée **`IA_CLASSIFY_BACKEND`** : `stub` (défaut — `classifyStub` keyword matching + taxonomie 3036), `word2vec` (Phase 6 — POST `/api/classify` Go, cosine similarity, ne pose pas `classification`), ou `llm` (Phase 7 — même endpoint Go, qui route en interne vers Mistral via `CLASSIFY_BACKEND=llm` + `MISTRAL_API_KEY` ; pose `classification` 3036 quand le `prompt_ia` du nœud l'invoque, fallback automatique sur word2vec en cas d'échec LLM). Player UI is `src/routes/sessions/[slug=number]/ScriptedPlayer.svelte`, rendered in place of `MainGraph.svelte` when `scenario.engine === 'scripted'`.
...
- Required keys: `PUBLIC_DB_URL`, `DB_URL`, optionally `IA_SERVER_URL`, `IA_CLASSIFY_BACKEND` (=`stub` (défaut) | `word2vec` | `llm` — sélectionne le classifieur scripted), `CSRF_CHECK_ORIGIN`. CSRF trusted origins are only enforced when `CSRF_CHECK_ORIGIN=true`. Côté Go (Phase 7) : `CLASSIFY_BACKEND=word2vec|llm` (défaut `word2vec`), `MISTRAL_API_KEY` (requis si llm), `MISTRAL_MODEL` (défaut `mistral-small-latest`), `MISTRAL_API_URL` (optionnel — override l'endpoint Mistral, défaut `https://api.mistral.ai/v1/chat/completions`). Si le LLM échoue (key absente, HTTP, timeout), le handler Go retombe automatiquement sur word2vec.
